...
Chat API at `app/api/chat/route.ts` uses Vercel AI SDK with gateway routing (grok-3-mini primary, Codex/GPT fallbacks). Fetches GitHub profile + repos for system context. Blog-contextual mode passes full article markdown. Client-side chat history stored in IndexedDB via Dexie (`lib/db/index.ts`).
