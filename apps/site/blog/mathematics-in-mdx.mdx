...
This companion article to [latex-mdx-next](https://github.com/claycurry/latex-mdx-next) aims to dissolve the XY problem
that motivated the creation.
the root 
problem solved 
 XY problem 
...
, put simply, because 
because the topic of technical and scientific communication over the web is utterly fucking fascinating.

 explores the challenges (and 
solutions) encountered in delivering technical and scientific content over the web.
solutions) encountered in 



After translating problems into requirements, the article compares trade-off characterizing alternative technology stacks and shares guidance on intrgrating [KaTeX](https://katex.org/) and the
