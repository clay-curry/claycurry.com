    ...
    <div className={`my-6 border overflow-hidden px-4 ${className ?? ""}`}>
      <Carousel opts={{ align: "center", loop: true }} setApi={setApi}>
        <CarouselContent className="-ml-10">
          {images.map((img, i) => (
            <CarouselItem key={i} className="basis-auto pl-10">
              <div
                className={`relative h-[420px] md:h-[520px] transition-opacity duration-300 ${i === activeIndex ? "opacity-100" : "opacity-40"}`}
              >
                <Image
                  src={img.src}
                  alt={img.alt ?? ""}
