...
import { useBlogChatContext } from '@/lib/hooks/use-chat'

export function AskQuestionBubble() {
  const [question, setQuestion] = useState('')
  const { setPrompt, setIsDialogOpen } = useBlogChatContext()
    ...
    <div className="fixed bottom-4 sm:bottom-6 left-1/2 -translate-x-1/2 z-50 w-full max-w-md px-3 sm:px-4">
      <form onSubmit={handleSubmit} className="relative">
        <input
          ...
          className="w-full pl-4 pr-14 py-3 bg-background border border-border rounded-2xl text-foreground placeholder:text-muted-foreground focus:border-accent focus:ring-2 focus:ring-accent/20 focus:outline-none transition-all text-sm shadow-lg"
        />
