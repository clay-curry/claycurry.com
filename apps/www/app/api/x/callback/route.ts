...
export const GET = withDebug(
  ...
  async (request: NextRequest, debug) => {
    ...
    try {
      ...
      debug.error("redis state retrieval failed", { error: String(err) });
      return NextResponse.json(
        {
          error:
    ...
    return Effect.runPromise(
      Effect.gen(function* () {
        ...
        debug.log("token exchanged successfully");
        ...
        debug.log("token ownership verified", {
          username: authenticatedOwner.username,
        });
        ...
        if (
          ...
          debug.error("owner mismatch", {
            authenticated: authenticatedOwner.username,
            resolved: resolvedOwner.username,
          });
          return NextResponse.json(
            {
        ...
        Effect.catchAll((error) => {
          ...
          debug.error("oauth callback failed", {
            code: errCode,
