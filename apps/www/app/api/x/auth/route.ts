...
export const GET = withDebug(
  ...
  async (request: NextRequest, debug) => {
    ...
    debug.log("oauth flow initiated", { provider: "x" });
    debug.log("pkce challenge generated");
    ...
    try {
      ...
      debug.log("state stored in redis", { ttl: 300 });
    } catch (err) {
      console.error("Failed to store OAuth state:", err);
      debug.error("redis state storage failed", { error: String(err) });
      return NextResponse.json(
