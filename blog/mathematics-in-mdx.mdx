...
Technical writers should be able to write this:

```{}
## Pythagorean Theorem

Any right triangle with legs of length \(a\) and \(b\), and hypotenuse of length \(c\) satisfies the below relationship:
...
- [Developer's Guide to Mathematical Typography in MDX](#developers-guide-to-mathematical-typography-in-mdx)
  - [Problem](#problem)
  - [Requirements / Scope](#requirements--scope)
  - [Pre-fetching versus Pre-rendering Mathematics](#pre-fetching-versus-pre-rendering-mathematics)
  - [The Unified Pipeline](#the-unified-pipeline)
    - [remark-math: Recognizing Math Syntax](#remark-math-recognizing-math-syntax)
    - [From Markdown to HTML](#from-markdown-to-html)
  - [The Input Problem: Which LaTeX?](#the-input-problem-which-latex)
    - [KaTeX's Supported Subset](#katexs-supported-subset)
    - [Practical Guidance](#practical-guidance)
  - [The Output Problem: HTML+CSS vs SVG vs MathML](#the-output-problem-htmlcss-vs-svg-vs-mathml)
    - [HTML+CSS (KaTeX Default)](#htmlcss-katex-default)
    - [SVG (MathJax Default)](#svg-mathjax-default)
    - [MathML (Native Browser)](#mathml-native-browser)
    - [The Accessibility Angle](#the-accessibility-angle)
  - [The Tradeoff: Bundle Size vs Rendering Speed vs Features](#the-tradeoff-bundle-size-vs-rendering-speed-vs-features)
    - [KaTeX](#katex)
    - [MathJax](#mathjax)
    - [When Each Makes Sense](#when-each-makes-sense)
  - [Recommended Stack for Next.js + MDX](#recommended-stack-for-nextjs--mdx)
    - [Dependencies](#dependencies)
    - [next.config.ts](#nextconfigts)
    - [Loading KaTeX CSS](#loading-katex-css)
    - [mdx-components.tsx](#mdx-componentstsx)
    - [Example MDX File](#example-mdx-file)
    - [Using MathJax Instead](#using-mathjax-instead)
    - [Rendered Output Comparison](#rendered-output-comparison)
  - [Code Splitting and Performance](#code-splitting-and-performance)
    - [Conditional CSS Loading](#conditional-css-loading)
    - [Preloading Critical Fonts](#preloading-critical-fonts)
    - [Inlining Critical CSS](#inlining-critical-css)
  - [What This Article Doesn't Cover](#what-this-article-doesnt-cover)
...
The term "pre-fetching" refers to the process of delivering content from pages reachable from the current page in
non-blocking fashion before users actually navigate to them. This stores frequently-accessed content inside in-memory
cache, using the [locality-of-reference](https://en.wikipedia.org/wiki/Locality_of_reference) principle to eliminate
[latency](https://developer.mozilla.org/en-US/docs/Web/Performance/Guides/Understanding_latency) overhead incurred from
disk reads, processing, and awaiting packets to travel over networks.

In contrast, "pre-rendering"  (a.k.a. [static-site generation](https://en.wikipedia.org/wiki/Static_site_generator))
refers to the process that translates content and source code into browser-ready, target content before uploading the
...
> [!NOTE]  
> This article uses the terms ***build-time***, ***request-time***, and
> ***load-time*** refer to distinct moments in the lifecycle of web content delivery:
>
> - ***build-time*** refers to the moment in time when the developer compiles and deploys the site to the server.
>   - happens before artifacts are uploaded to a web server.
> - ***request-time*** refers to moment in time when a web server software (e.g., Apache, Nginx) establishes a TCP
connection with
>   - happens immediately after the web server host machine consumes packets from a client.
> - ***load-time*** refers to the moment in time a browser client fires the [load event](https://developer.mozilla.org/en-US/docs/Web/API/Window/load_event)
>   - happens after the browser has downloaded all resources (HTML, CSS, JavaScript, images, fonts, etc.) required to render a page.


This technique contrasts with server-side rendering and client-side rendering in the following ways:

| Rendering Technique | When Content is Produced | Where Content is Produced | Description |
| ------------------- | ----------------------- | ------------------------ | ----------- |
| Pre-rendering (SSG) | build-time | Developer's machine / CI server | Content is produced before deployment, so the server serves fully-formed pages to the client. |
