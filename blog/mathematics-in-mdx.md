...
from in-memory cache makes use of the [locality-of-reference](https://en.wikipedia.org/wiki/Locality_of_reference) 
principle. Consequently, pre-fetching has added benefit of reducing load on servers and networks, preserving 
functionality during network outages, and reducing failure points during application usage.

[^2] Pre-rendering technique contrasts with server-side rendering and client-side rendering in the following ways:

| Rendering Technique | When Content is Produced | Where Content is Produced | Description |
| ------------------- | ----------------------- | ------------------------ | ----------- |
| Pre-rendering (SSG) | build-time[^3] | Developer's machine / CI server | Content is produced before deployment, so the server serves fully-formed pages to the client. |
| Server-side rendering (SSR) | request-time | Server | Content is produced on the server when a user requests a page, so the server sends fully-formed pages to the client. |
| Client-side rendering (CSR) | load-time | Client's browser | Content is produced in the browser after the initial HTML shell is loaded, so the server sends minimal HTML and JavaScript to the client. |
